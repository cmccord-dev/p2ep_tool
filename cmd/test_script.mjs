...
export const command = [`test_script start [end]`];
  ...
  for (let i = start; i <= end; i++) {
    ...
    files.forEach((f, o) => {
      ...
      if (res && res.type == "script") {
        ...
        let compiled = scene_script.compile_script(script);
        console.assert(
          compiled.compare(res.data)==0,
          `Scripts are equal ${i}_${o}`
        );
        script.file = i;
        script.file_num = o;
        // fs.writeFileSync(
        ...
        // );
        // fs.writeFileSync(`${argv.out}/${i}_${o}.bin`, compiled);
        // fs.writeFileSync(`${argv.out}/${i}_${o}_res.bin`, res.data);
      }
    });
  }
};
